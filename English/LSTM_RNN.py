...
if __name__ == "__main__":
	...
	plt.savefig("Images/LSTM_2_ENG.jpg")
	...
	pr_te = metrics.precision_score(y_te, prediction_te[:, 1], average='micro')
	f1_te = metrics.f1_score(y_te, prediction_te[:, 1], average='micro')
	re_te = metrics.recall_score(y_te, prediction_te[:, 1], average='micro')

	pr_tr = metrics.precision_score(y_tr, prediction_tr[:, 1], average='micro')
	f1_tr = metrics.f1_score(y_tr, prediction_tr[:, 1], average='micro')
	re_tr = metrics.recall_score(y_tr, prediction_tr[:, 1], average='micro')

	# score
	score_train = metrics.accuracy_score(y_tr, prediction_tr[:, 1])
	score_test = metrics.accuracy_score(y_te, prediction_te[:, 1])
	...
	fpr, tpr, _ = metrics.roc_curve(y_te.ravel(), prediction_te[:, 1].ravel())
	...
	plt.savefig("Images/LSTM_2_ROC")

	submission[["toxic", "severe_toxic", "obscene", "threat", "insult", "identity_hate"]] = prediction_te
	submission.to_csv('Predictions/submission16.csv', index=False)
